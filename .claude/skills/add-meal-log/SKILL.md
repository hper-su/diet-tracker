...
4. **結果を確認する。** 1分ほど待ってから(Bash の `sleep 60` を `run_in_background` で実行)、
   ...
   「登録済みのためスキップ(N件)」が出ていたら、その件は既に登録されている(二重登録を
   自動で防ぐ)。全件スキップなら本番登録は不要なので、その旨をユーザーに伝えて終える。
   一部だけスキップされた場合は、どれが登録済みだったかを報告に含める。

5. **本番登録する。** 手順3と同じ内容で `dry_run` を `"false"` にして実行し、
   ログの最後に「登録しました(N件)。」と出ていることを確認する。
...
- 重複チェックあり: 同じお客様・同じ日付に、区分・品目名・数量・kcal/PFCがすべて一致する
  記録が既にあれば、その件はスキップされる(区分が違えば別の記録として登録される)。
  同じ依頼を2回実行しても二重登録にはならない。
- 同じものを本当にもう1回食べた等で、登録済みと同じ内容をあえて追加したい場合だけ、
  `inputs` に `"allow_duplicates": "true"` を加えて実行する(ユーザーの明示的な指示がある場合のみ)。
- 数値や品目名を少しでも変えると別の記録とみなされる。内容の修正は二重登録になるので、
  誤登録の修正・削除はアプリの画面から行うようユーザーに伝える。
